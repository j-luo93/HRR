...
from google3.third_party.tensorflow_lingvo.core import py_utils
from google3.third_party.tensorflow_lingvo.core import quant_utils
...
class EmbeddingLayer(base_layer.LayerBase):
  ...
  def Params(cls):
    ...
    p.Define('partition_strategy', 'mod',
             'Argument for partition strategy, used in tf.nn.embedding_lookup')
    p.Define(
        'actual_shards', 0,
        'Actual number of shards used. If nonzero, forced to have this many shards.'
    )
    p.Define('trainable', True, 'whether this is trainable')
    return p
  ...
  def __init__(self, params):
    ...
    if p.actual_shards > 0:
      actual_shards = p.actual_shards
    ...
    with tf.variable_scope(p.name):
      for i in range(actual_shards):
        vi, vi_var = py_utils.CreateVariable('var_%d' % i, w_pc, trainable=p.trainable)
  ...
  def EmbLookup(self, theta, ids):
    ...
    embs = tf.nn.embedding_lookup(theta.wm, tf.reshape(ids, [-1]), partition_strategy=p.partition_strategy)
